...
func InsertarTitular(p Titular) (int, error) {
	...
	_, err := database.DB.Exec(query, p.Nombre, p.Apellidos, p.Sexo.String(),
		p.Dni, p.FechaNacimiento, p.Telefono, p.Pais, p.Provincia, p.Distrito, p.TipoVia.String(), p.Direccion, p.Referencia, p.Ocupacion, p.NombreEmpresa, p.DireccionEmpresa, p.IngresoPromedio,p.EsPostulante)
	if err != nil {
